...
class BaselineObjectives:
    ...
    def __init__(self, env, bb_model, enc, enc_dataset, n_var, cfmode):
        self.env = env
    ...
    def get_ind_rews(self, fact, cf, target_action, prev_actions, cummulative_reward=None):
    ...
    def get_reward(self, fact, cf, target_action, prev_actions, cummulative_reward=0):
        objectives = self.get_objectives(fact)
        contraints = self.get_constraints(fact, target_action, prev_actions)
    ...
    def sparsity(self, x, fact):
        return ((sum(fact[:25] != x[:25]) * 1.0).item())/self.n_var
