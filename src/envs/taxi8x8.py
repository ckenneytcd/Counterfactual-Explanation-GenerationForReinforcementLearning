...
class TaxiEnv(Env):
    ...
    * v0: Initial versions release
    ...
    def __init__(self, render_mode: Optional[str] = None):
        ...
        self.locs = locs = [(0, 0), (0, 7), (7, 0), (7, 6)]
    ...
    def action_mask(self, state: int):
        ...
        if taxi_row < self.num_rows:
            mask[0] = 1
        if taxi_row > 0:
            mask[1] = 1
        if taxi_col < self.num_columns and self.desc[taxi_row + 1, 2 * taxi_col + 2] == b":":
            mask[2] = 1
    ...
    def render_state(self, state):
        ...
        #     outfile.write(
        #         f"  ({['South', 'North', 'East', 'West', 'Pickup', 'Dropoff'][lastaction]})\n"
        #     )
        # else:
        #     outfile.write("\n")
    ...
    def check_done(self, state):
        #Passanger is at destination
        # if(state[3] == 0 or state[3] == 1):
        #     dstrow = 0
        # if(state[3] == 2 or state[3] == 3):
        #     dstrow = 4
        # if(state[3] == 0 or state[3] == 2):
        #     dstcol = 0
        # if(state[3] == 1 or state[3] == 3):
        #     dstcol = 4
        
        # if(state[0] == dstrow and state[1] == dstcol and state[2] == 4):
        # #     print("!!!DONE")
        if state[2] == state[3]:
            return True
        else:
    ...
    def realistic(self, x, f, prev_actions):
        ...
        if(prev_actions and prev_actions[-1] == 4):
            if not((f[2] == 0 and x[0] == 0 and x[1] == 0)):
                if not(f[2] == 1 and x[0] == 0 and x[1] == 7):
                    if not(f[2] == 2 and x[0] == 7 and x[1] == 0):
                        if not(f[2] == 3 and x[0] == 7 and x[1] == 7-1):
        ...
        if(prev_actions and prev_actions[-1] == 5):
            if not((f[3] == 0 and x[0] == 0 and x[1] == 0)):
                if not(f[3] == 1 and x[0] == 0 and x[1] == 7):
                    if not(f[3] == 2 and x[0] == 7 and x[1] == 0):
                        if not(f[3] == 3 and x[0] == 7 and x[1] == 7-1):
